...
public class UserControllerV2 extends BaseControllerV2 {
    ...
    public @ResponseBody User getDetails(@PathVariable Long id, @RequestAttribute("user") User authUser, HttpServletResponse response) {
    ...
    public @ResponseBody User updateDetails(@PathVariable Long id, @RequestBody UserUpdateDto dto, @RequestAttribute("user") User authUser, HttpServletResponse response) {
        var user = userService.getUser(id);
        ...
        response.setStatus(SC_OK);
        return userService.updateDetailsV2(user, authUser, dto, adminRequest);
    }
